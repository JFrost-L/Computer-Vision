...
class ResNet152Model(nn.Module):
    def __init__(self, num_classes=10, embedding_dim=128):
        ...
        self.embedding_size = embedding_dim  # Add the embedding_size attribute
        self.fc = nn.Linear(embedding_dim, num_classes)  # Additional fully connected layer for classification
        self.name = 'resnet152d'  # Add the name attribute

    def forward(self, x):
        embeddings = self.model(x)
        embeddings = F.relu(embeddings)
        output = self.fc(embeddings)
        return embeddings, output  # Return embeddings and output
        ...
        with torch.no_grad():
            with tqdm(total=len(self.test_loader), desc=f"Testing", unit="batch", leave=False) as pbar:
                for batch_idx, (anchor, positive, negative, labels) in enumerate(self.test_loader):
                    ...
                    anchor_output, _ = self.model(anchor)
                    positive_output, _ = self.model(positive)
                    negative_output, _ = self.model(negative)
                    embeddings = torch.cat((anchor_output, positive_output, negative_output))
    ...
    def _extract_features_labels(self, model, loader):
        ...
        with torch.no_grad():
            for inputs, _, _, label in loader:
                ...
                outputs, _ = model(inputs)  # Extract embeddings only
...
if __name__ == '__main__':
    ...
    trainTransform = A.Compose([
        A.Resize(120, 120),  # 이미지 크기 조정
        A.Rotate(limit=30, p=0.5),  # 최대 30도 회전
        A.HorizontalFlip(p=0.5),  # 수평 뒤집기
        A.ColorJitter(brightness=0.1, contrast=0.1, saturation=0.2, p=0.5),  # 밝기, 대비, 채도 조절
        A.Affine(scale=(0.8, 1.2), translate_percent=(0.1, 0.1), rotate=(-30, 30), shear=(-10, 10), p=0.5),  # 여러 가지 변환
        A.GaussianBlur(p=0.5),  # 가우시안 블러
        A.Sharpen(alpha=(0.2, 0.5), lightness=(0.5, 1.0), p=0.5),  # 샤프닝 추가
        A.IAAPiecewiseAffine(p=0.5),  # 이미지 왜곡
        A.IAASharpen(p=0.5),  # 이미지 샤프닝
        A.Equalize(p=0.5),  # 밝기 평활화 추가
        A.CoarseDropout(p=0.5, max_holes=1, max_height=30, max_width=30, min_holes=1, min_height=10, min_width=10),  # 이미지 일부 삭제
        A.RandomResizedCrop(height=120, width=120, scale=(0.8, 1.0), p=0.5),  # 무작위 크기 조정 및 자르기
