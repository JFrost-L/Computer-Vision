...
class ModelTrainer:
    def __init__(self, model, device, dataset, test_loader, mean, std, epochs=30, learning_rate=0.00001, label_smoothing=0.1):
    ...
    def train_and_evaluate(self):
        targets = [self.dataset.targets[i] for i in range(len(self.dataset.targets))]
...
if __name__ == '__main__':
    ...
    model = ResNet152Model(num_classes=10)

    trainer = ModelTrainer(model, device, image_loader.full_dataset, test_loader, image_loader.mean, image_loader.std, epochs=epochs, learning_rate=learning_rate)
