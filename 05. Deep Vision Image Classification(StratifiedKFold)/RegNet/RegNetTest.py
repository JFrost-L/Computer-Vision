...
class RegNetYModel(nn.Module):
    def __init__(self, num_classes=10):
        super(RegNetYModel, self).__init__()
        self.model = timm.create_model('regnetz_e8', pretrained=True)
        in_features = self.model.head.fc.in_features  # 모델의 끝 부분에 있는 Fully Connected 레이어의 입력 피쳐 수를 가져옴
        self.model.head.fc = nn.Linear(in_features, num_classes)  # 모델의 끝 부분에 있는 Fully Connected 레이어를 새로운 레이어로 대체
...
if __name__ == '__main__':
    ...
    model_pattern = "best_model_regnetz_e8_epoch_*.pth"
    batch_size = 64
