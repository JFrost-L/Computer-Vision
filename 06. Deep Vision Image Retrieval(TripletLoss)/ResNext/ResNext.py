...
class ResNeXtModel(nn.Module):
    def __init__(self, num_classes=10, embedding_dim=128):
        ...
        self.fc = nn.Linear(embedding_dim, num_classes)  # 추가된 fully connected layer
        self.name = 'resnext101_64x4d '

    def forward(self, x):
        x = self.model(x)
        x = F.relu(x)  # 추가된 ReLU activation
        x = self.fc(x)
        return x
...
class ModelTrainer:
    ...
    def _calculate_retrieval_accuracy(self, model):
        ...
        train_features, train_labels = self._extract_features_labels(model, self.train_loader)
        ...
        print(f"Retrieval Accuracy = {total_correct_labels}/{total_labels}*100 : retrieval_accuracy")
        print()
    ...
    def _extract_features_labels(self, model, loader):
        ...
        with torch.no_grad():
            for inputs, _, _, label in loader:  # Triplet 데이터셋에 맞게 수정
                inputs = inputs.to(self.device)
                outputs = model(inputs).cpu().numpy()
                features.extend(outputs)
                labels.extend(label)  # 레이블을 직접 추가
...
if __name__ == '__main__':
    ...
    # 전체 데이터셋을 불러옵니다.
    full_dataset = TripletImageDataset(train_data_path, transform=None)  # 초기에는 transform 없이 로드

    # 전체 데이터셋을 학습용과 검증용으로 나눕니다.
    train_size = int(0.8 * len(full_dataset))
    val_size = len(full_dataset) - train_size
    train_dataset, val_dataset = torch.utils.data.random_split(full_dataset, [train_size, val_size])

    # 각 데이터셋에 대해 변환을 적용합니다.
    train_dataset.dataset.transform = trainTransform  # 학습 데이터셋에 trainTransform 적용
    val_dataset.dataset.transform = transform         # 검증 데이터셋에 transform 적용
    ...
    # 트레이너 초기화 및 학습/평가 실행
    trainer = ModelTrainer(model, device, train_loader, val_loader, test_loader, epochs=epochs, learning_rate=learning_rate)
