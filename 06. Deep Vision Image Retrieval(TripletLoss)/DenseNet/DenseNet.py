...
class ModelTrainer:
    def __init__(self, model, device, train_loader, val_loader, test_loader, epochs=30, learning_rate=0.00001, fold=0):
        ...
        self.train_losses = []
        self.val_losses = []
        self.best_val_retrieval_accuracy = 0.0  # Initialize best validation retrieval accuracy
        self.best_epoch = 0
        self.fold = fold  # Add fold information
    ...
    def train_and_evaluate(self):
        for epoch in range(self.epochs):
            ...
            # Check if validation retrieval accuracy is increased
...
if __name__ == '__main__':
    ...
    # Test each fold's best model and store results
    for fold_result in all_fold_results:
        fold = fold_result['fold']
        print(f"Testing fold {fold} best model")
        
        model = DenseNetModel()
        trainer = ModelTrainer(model, device, None, None, test_loader, epochs=epochs, learning_rate=learning_rate, fold=fold)  # train_loader와 val_loader는 None으로 설정
