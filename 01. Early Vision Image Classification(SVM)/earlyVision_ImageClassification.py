...
# 모델 및 메타데이터 저장 함수
def save_model_and_metadata(model, scaler, pca, max_length, filename='best_svm_model.pkl'):
    joblib.dump({'model': model, 'scaler': scaler, 'pca': pca, 'max_length': max_length}, filename)
...
def main_torch():
    ...
    print(f"Preprocessing time: {preprocessing_end_time - preprocessing_start_time} seconds")
    ...
    for train_index, val_index in skf.split(features_pca, labels_encoded):
        train_features, val_features = features_pca[train_index], features_pca[val_index]
        train_labels, val_labels = labels_encoded[train_index], labels_encoded[val_index]
        
        svm = SVC(kernel='linear', C=1)
        svm.fit(train_features, train_labels)
        
        val_pred_labels = svm.predict(val_features)
        val_accuracy = accuracy_score(val_labels, val_pred_labels)
        
        print(f"Fold {fold_index} - Validation Accuracy: {val_accuracy}")
        print(classification_report(val_labels, val_pred_labels, target_names=le.classes_))
        
        if val_accuracy > best_accuracy:
            best_accuracy = val_accuracy
            best_model = svm
            save_model_and_metadata(best_model, scaler, pca, max_length, 'best_svm_model.pkl')
        
        cm = confusion_matrix(val_labels, val_pred_labels)
        plot_confusion_matrix(cm, le.classes_, title=f'Confusion matrix for validation set (Fold {fold_index})')
    ...
    if best_model is not None:
        best_model, scaler, pca, max_length = load_model_and_metadata('best_svm_model.pkl')
