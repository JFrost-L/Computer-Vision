...
# Function to find top N neighbors within the same predicted class
...
# Main function for SVM training and multi-class image retrieval
def main_torch():
    ...
    for train_index, val_index in skf.split(features, y_encoded):
        ...
        val_pred_labels = svm.predict(X_val_pca)
        val_accuracy = accuracy_score(y_val, val_pred_labels)
        
        print(f"Fold {fold_num} Validation Accuracy: {val_accuracy}")
        print(classification_report(y_val, val_pred_labels, target_names=le.classes_))
